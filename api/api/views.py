...
class EvaluateMarks():
  ...
  def __init__(self,
    ...
    self.totalMarks = database[database.q_category ==
                               self.question_no]['totalMarks'].item()
    self.toxic_examples = [
  ...
  def _semantic_check(self):
    ...
    answersAll = answers.get_all()
    database = pd.DataFrame(answersAll)
    embeds = co.embed(texts=database[database.q_category == self.question_no][[
  ...
  def _gen_grammar_check(self):
    ...
    for i in range(self.gen_trails):
      ...
      sim_value = cosine_similarity(X=[embeds[0]],
                                    Y=[embeds[1]],
                                    dense_output=True)
  ...
  def _class_grammar_check(self):
    """
        """
    response = co.classify(model='cdb39157-6b82-4cb4-92c5-9e6037623d79-ft',
                           inputs=[f"{self.query}"])
  ...
  def _custom_toxic_check(self):
    ...
    sentences = self.query.lower().rstrip('. ').split('.')
    for i in sentences:
      response = co.classify(model='8cec2377-0f7f-4557-81a4-7abc7dea3828-ft',
                             inputs=[f"{i}"])
...
def add_answer():
  ...
  totalMarks = request.json.get('totalMarks')
...
def evaluate_answer():
  ...
  eval = EvaluateMarks(int(categoryQuery), query=answerQuery)
  marks = eval.evaluate()
  # print(marks)
...
def run_checks():
  ...
  return [
    "The question is " + format(
      ((1 - round(checks[0], 2)) * 100), '.2f') + "% similar to the answer",
    "The answer is grammatically correct"
    if checks[1] == 0 else "The answer is not grammatically correct",
    "The answer is not toxic" if checks[2] == 0 else "The answer is toxic",
    "The answer is not duplicate"
    if checks[3] == 0 else "The answer is not duplicate"
  ]
